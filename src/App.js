...
import { appReducer, AppContext } from './reducers/app_reducer.js'
...
function App() {
    ...
    useEffect(() => {
      userInfo.current.sub = 'U557c967ab81de347ad07418d0d9e9771' // for test
      userInfo.current.name = '王柏軒'
      userInfo.current.picture = 'https://profile.line-scdn.net/0hpQiB_gYYL35UOzzk6U5QKWh-IRMjFSk2LA43GyQ9dUgrCWEoPVk1HyQ4cx18W2koa19kHSVpcE96'
      userInfo.current.groupId = 'Ca856789c53ffc35da8c8cdeb521c47fd'
        if (!userInfo.current?.groupId) return
        const q = query(collection(db, userInfo.current.groupId), orderBy('createdAt', 'desc'), limit(5))
        const docRef = doc(db, userInfo.current.groupId, 'config')
        const unsubscribeGetConfigData = onSnapshot(
            docRef,
            async (docSnap) => {
                ...
                if (docSnap.exists()) {
                    ...
                    try {
                        const formattedDebt = await formatDebtRecords(data.users, data.records, userInfo.current.sub)
        ...
        const unsubscribeGetRecentRecords = onSnapshot(
            q,
            (snapshot) => {
                dispatch({ type: 'set_loading', value: true })
                if (snapshot.empty) {
    ...
    return (
        <AppContext.Provider value={{ state, dispatch, userInfo }}>
            <div className="App">
                <Loading />
                <AddRecord />
                {state.recordMenu !== null && <SetRecords />}
                <Prompt />
                ...
                <div className="container p-4" ref={containerRef} style={{ maxWidth: '40rem' }}>
                    ...
                    <div>
                        <div className="text-start">
                            ...
                            <p className="fw-light m-0">在這裡將設定算錢工具機器人和權限</p>
                        </div>
