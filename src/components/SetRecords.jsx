        ...
        <div className="d-flex align-items-center justify-content-center" style={{ position: 'fixed', width: '100vw', height: '100vh', backgroundColor: 'rgba(0, 0, 0, 0.8)', zIndex: 2 }}>
            <div className="bg-white rounded shadow p-3" style={{ maxWidth: 'calc(100vw - 2rem)', width: '25rem', maxHeight: 'calc(100vh - 5rem)', overflow: 'auto' }}>
                ...
                <div className="text-start mt-2">
                    {editMode ? <input type="text" className="form-control w-50" placeholder="標題" value={menuChange.title} onChange={(e) => {
                        setMenuChange({ ...menuChange, title: e.target.value })
                    }} /> : <p className="fs-4 fw-light mb-1 d-inline">標題：<strong>{menuChange.title || '未命名'}</strong></p>
                    }
                    {
                        editMode ? <input type="text" className="form-control mt-1" placeholder="描述" value={menuChange.description} onChange={(e) => {
                            setMenuChange({ ...menuChange, description: e.target.value })
                        }} /> : <p className="fs-6 text-secondary mb-0">{menuChange.description || '未設定'}</p>
                    }
                    <button className="btn btn-outline-dark btn-sm mt-1" onClick={() => setEditMode(!editMode)}>編輯</button>
                    <div className="d-flex flex-column">
                        {menuChange.records.map((item, index) => <div key={item.id}>
                            <hr />
                            <div className="d-flex align-items-center">
                                ...
                                <div className="text-center" style={{ width: '4rem' }}>
                                    ...
                                    <p className="m-0" style={{ fontSize: '12px' }}>{getUserInfo(users, item.borrower).name}</p>
                                </div>
                                ...
                                {
                                    editMode ? <div>
                                        <input type="text" className="form-control" placeholder="名稱" value={menuChange.records[index].remark} onChange={(e) => {
                                            updateRecordRemark(setMenuChange, index, e.target.value)
                                        }} />
                                        <input type="number" className="form-control mt-1" placeholder="金額" value={menuChange.records[index].debt} onChange={(e) => {
                        ...
                        {
                            newTabOpenState ? <div className="p-2 rounded bg-light mt-2 shadow-sm border">
                                ...
                                <div className="mt-3 d-flex justify-content-start align-items-center gap-2 flex-wrap">
                                    {
                                        users.map(item => <div className={`text-center p-1 hover-darken border rounded ${addBorrower === item.uid ? 'bg-info-subtle' : ''}`} key={item.uid} onClick={() => {
                                ...
                                <div className="mt-3 d-flex justify-content-start align-items-center gap-2 flex-wrap">
                                    {
                                        users.map(item => <div className={`text-center p-1 hover-darken border rounded ${addDebtor === item.uid ? 'bg-info-subtle' : ''}`} key={item.uid} onClick={() => {
