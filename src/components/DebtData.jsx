...
import useClientWidth from '../hooks/useClientWidth.js'
...
export default function DebtData({ containerRef }) {
    ...
    const [imgWidth, setImgWidth] = useState(
        width / context.state.debtData.length > 30 ? 30 : width / context.state.debtData.length,
    )
    const [rawData, setRawData] = useState(
        context.state.debtData.filter((item) => item.debt !== 0).map((item) => item.debt),
    )
    const [labels, setLabels] = useState(
        context.state.debtData.filter((item) => item.debt !== 0).map((item) => item.name),
    )
    useEffect(() => {
    ...
    return (
        <div className="mb-3">
            <div className="text-start">
                ...
                <p className="fw-light">紅色為欠你錢、綠色為你欠別人錢</p>
